...
function Pedometer() {
  ...
  this.createTable = function (lWindow) {
    this.acc_norm = new Array(125);
    this.stepArr = new Array(125);
  };
  ...
  this.onStep = function (acc, cb) {
    ...
    var isOverThreshold =
      acc[acc.length - 1] >= this.threshold &&
      acc[acc.length - 2] < this.threshold; // if the acceleration goes over the threshold and the previous was below this threshold
  ...
  this.setDistanceGPS = function () {
    ...
    this.distGPS = Math.acos(
      Math.sin(lat1) * Math.sin(lat2) +
        Math.cos(lat1) * Math.cos(lat2) * Math.cos(lon1 - lon2)
    );

    this.timeGPS = Math.abs(
      this.GPSposition[0].timestamp - this.GPSposition[1].timestamp
    );
  ...
  this.onSpeed = function () {
    ...
    if (Boolean(this.isGPSEnabled) && Boolean(this.isGPSReceived)) {
      if (
        this.timeGPS < 6000 &&
        isNaN(this.distGPS) == 0 &&
        isNaN(speedAcc) == 0 &&
        isNaN(this.timeGPS) == 0
      ) {
        if (this.timeGPS > 0) {
    ...
    if (
      ...
      if (isNaN(this.meanSpeed) == 0) {
        this.meanSpeed =
          (this.meanSpeed * (this.countStep - 1) + this.speed) / this.countStep;
      } else {
  ...
  this.onDraw = function (context, widthCanvas, heightCanvas) {
    ...
    for (var k = 0; k <= vitesseLimite; k++) {
      ...
      if (k % 5 == 0) {
        ...
        context.fillText(
          k,
          widthCanvas / 2 +
            radiusCircle * 1.1 * Math.cos(angleCanvas) -
            context.measureText(k).width / 2,
          heightCanvas / 2 + radiusCircle * 1.1 * Math.sin(angleCanvas)
        );
      } else {
        context.strokeStyle = "rgb(0, 0, 0)";
      }
      context.moveTo(
        widthCanvas / 2 + (radiusCircle / 1.05) * Math.cos(angleCanvas),
        heightCanvas / 2 + (radiusCircle / 1.05) * Math.sin(angleCanvas)
      ); // 1st point
      context.lineTo(
        widthCanvas / 2 + radiusCircle * Math.cos(angleCanvas),
        heightCanvas / 2 + radiusCircle * Math.sin(angleCanvas)
      ); // 2nd point
      context.stroke();
    }
    ...
    context.fillText(
      "km/h",
      widthCanvas / 2 - context.measureText("km/h").width / 2,
      (heightCanvas * 1) / 3
    );
    ...
    if (isNaN(this.speed) == 1) {
      ...
      angleCanvas =
        (Math.PI / 180) * (150 + (vitesseLimite * 240) / vitesseLimite);
    } else {
      angleCanvas =
        (Math.PI / 180) *
        (150 + (Math.round((this.speed / 1000) * 3600) * 240) / vitesseLimite);
    }
    context.moveTo(widthCanvas / 2, heightCanvas / 2); // 1st point
    context.lineTo(
      widthCanvas / 2 + radiusCircle * 0.9 * Math.cos(angleCanvas),
      heightCanvas / 2 + radiusCircle * 0.9 * Math.sin(angleCanvas)
    ); // 2nd point
    context.stroke();
    ...
    context.arc(
      widthCanvas / 2 + radiusCircle * 0.95 * Math.cos(angleCanvas),
      heightCanvas / 2 + radiusCircle * 0.95 * Math.sin(angleCanvas),
      radiusCircle * 0.05,
      0,
      2 * Math.PI
    );
    ...
    if (isNaN(this.countStep) == 0) {
      context.fillText(
        this.countStep,
        widthCanvas / 2 - context.measureText(this.countStep).width / 2,
        (heightCanvas * 5) / 6
      );
    } else {
      context.fillText(
        "Error",
        widthCanvas / 2 - context.measureText("Error").width / 2,
        (heightCanvas * 5) / 6
      );
    }
